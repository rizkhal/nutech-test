<template>
  <main>
    <div class="p-4 block sm:flex lg:mt-1.5 bg-white items-center justify-between border-b border-gray-200">
        ...
        <div class="sm:flex">
          ...
          <div class="flex items-center space-x-2 sm:space-x-3 ml-auto">
            <template v-for="(action, index) in actions" :key="index">
              <button type="button" @click.prevent="$emit(action.action)" class="w-1/2 text-white bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:ring-cyan-200 font-medium inline-flex items-center justify-center rounded-lg text-sm px-3 py-2 text-center sm:w-auto">
                {{ action.text }}
              </button>
            </template>

            <div class="ml-3 relative" v-if="Object.keys(data.fields).length">
              <Dropdown :width="64" align="right" content-classes="bg-white p-2">
                <template #trigger>
                  <span class="inline-flex rounded-md">
                    <button type="button" class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-white bg-cyan-600 hover:text-cyan-200 focus:outline-none transition ease-in-out duration-150">
                      Filter
                ...
                <template #content>
                  <div v-for="(field, key) in data.fields" :key="key">
                    <InputSelect v-if="field.select" :v-model="field.key" :label="data.fields[key].name" @change="handleOnSelectFilter($event, field.key)">
          ...
          <div class="shadow overflow-hidden">
            <table class="table-fixed min-w-full divide-y divide-gray-200">
              ...
              <tbody class="bg-white divide-y divide-gray-200" v-if="data.data.data.length">
                <tr v-for="(item, index) in data.data.data" :key="index" class="hover:bg-gray-100">
                  <template v-for="(column, i) in data.columns" :key="i">
                    ...
                    <td v-else-if="!column.blank" class="p-4 whitespace-nowrap text-base font-medium text-gray-900">
              ...
              <tbody class="bg-white divide-y divide-gray-200" v-else>
                <tr>
                  <td class="text-center p-4 text-gray-600" :colspan="data.columns.length">Kosong</td>
                </tr>
              </tbody>
            </table>
          </div>
  ...
  data() {
    return {
      ...
      params: {
        ...
        filters: this.data.filters.filters ? Object.keys(this.data.filters.filters).reduce((ac, a) => ({ ...ac, [a]: this.data.filters.filters[a] }), {}) : null,
      },
      filtersModels: this.data.fields ? Object.keys(this.data.fields).reduce((ac, a) => ({ ...ac, [a]: null }), {}) : null,
    };
  },
  methods: {
    handleSort(column) {
    ...
    handleOnSelectFilter(event, key) {
